...
PYBIND11_MODULE(_native, m) {
  m.doc() = "Native Aerospike connector for LMCache L2";

  py::class_<lmcache_aerospike::AerospikeNativeConnector>(
      m, "AerospikeNativeClient")
      .def(py::init<std::string, std::string, std::string, int, uint32_t,
                    uint32_t, uint32_t, std::string, size_t, size_t,
          ...
          LMCACHE_BIND_CONNECTOR_METHODS(
              lmcache_aerospike::AerospikeNativeConnector);
}
