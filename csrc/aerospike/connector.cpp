...
AerospikeNativeConnector::AerospikeNativeConnector(
    ...
    uint32_t default_ttl_seconds, std::string dtype,
    size_t target_segment_bytes, size_t max_record_bytes, std::string username,
    std::string password)
...
void AerospikeNativeConnector::do_single_get(WorkerAerospikeConn& conn,
                                             ...
                                             size_t len,
                                             size_t /*chunk_size*/) {
  std::string cache_key = native_key_to_cache_engine_key(key, dtype_);
  std::string meta_key = meta_user_key(cache_key);
  ...
  for (uint32_t i = 0; i < nseg; ++i) {
    std::string segment_key_i = segment_user_key(cache_key, i);
...
void AerospikeNativeConnector::do_single_set(WorkerAerospikeConn& conn,
  ...
  if (shard.nseg == 1) {
    put_meta_record(conn, meta_user_key(cache_key), shard, len, buf);
    return;
  }
  ...
  for (uint32_t i = 0; i < shard.nseg; ++i) {
    ...
    put_payload_record(conn, segment_user_key(cache_key, i),
                       static_cast<const char*>(buf) + start, chunk_len);
  }
  put_meta_record(conn, meta_user_key(cache_key), shard, len, nullptr);
}

bool AerospikeNativeConnector::do_single_exists(WorkerAerospikeConn& conn,
                                                const std::string& key) {
  std::string cache_key = native_key_to_cache_engine_key(key, dtype_);
  std::string user_key = meta_user_key(cache_key);
...
void AerospikeNativeConnector::shutdown_connections() {
  // The shared Aerospike client must outlive worker threads because queued
  // requests may still be draining after ConnectorBase sets stop_.
}
...
std::string AerospikeNativeConnector::native_key_to_cache_engine_key(
    const std::string& key, const std::string& dtype) {
  std::vector<std::string> parts = split(key, '@');
  if (parts.size() != 3 && parts.size() != 4) {
    throw std::runtime_error("native ObjectKey string must have 3 or 4 fields");
  }
  uint64_t kv_rank = parse_hex_u64(parts[1]);
  uint64_t world_size = (kv_rank >> 24) & 0xffU;
  uint64_t worker_id = (kv_rank >> 16) & 0xffU;
  if (world_size == 0) {
    world_size = 1;
  }
  std::string chunk_hash = trim_low_64_hex(parts[2]);

  std::ostringstream oss;
  oss << parts[0] << '@' << world_size << '@' << worker_id << '@' << chunk_hash
      << '@' << dtype;
  if (parts.size() == 4 && !parts[3].empty()) {
    oss << "@cache_salt%" << parts[3];
  }
  return oss.str();
}

std::string AerospikeNativeConnector::trim_low_64_hex(std::string hex) {
  if (hex.size() > 16) {
    hex = hex.substr(hex.size() - 16);
  }
...
uint64_t AerospikeNativeConnector::parse_hex_u64(const std::string& hex) {
  uint64_t value = 0;
  for (char c : hex) {
    value <<= 4;
    if (c >= '0' && c <= '9') {
      ...
      throw std::runtime_error("invalid hex digit in native key");
    }
  }
  return value;
}
